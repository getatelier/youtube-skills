...
def fetch_traffic_sources(
    service: Any, channel_id: str, video_ids: list[str], start_date: str, end_date: str
) -> list[dict[str, Any]] | dict[str, str]:
...
def get_channel_id(oauth_creds: Any) -> str | None:
...
def main() -> None:
    ...
    parser.add_argument(
        "--days", type=int, default=28, help="Number of days to look back (default: 28)"
    )
    parser.add_argument(
        "--metrics", type=str, default=None, help="Comma-separated metrics (default: standard set)"
    )
    parser.add_argument(
        "--traffic-sources", action="store_true", help="Include traffic source breakdown"
    )
    parser.add_argument("--daily", action="store_true", help="Include daily metric breakdown")
    ...
    try:
        ...
        if token_path.exists():
            ...
            print(json.dumps({"error": "OAuth token not found. Run auth first."}), file=sys.stderr)
            sys.exit(1)
    except Exception as e:
        print(json.dumps({"error": f"Failed to get channel ID: {e}"}), file=sys.stderr)
        sys.exit(1)
    ...
    if args.traffic_sources:
        traffic = fetch_traffic_sources(service, channel_id, args.video_ids, start_date, end_date)
        result["metrics"]["traffic_sources"] = traffic

    if args.daily:
        daily = fetch_daily_metrics(service, channel_id, args.video_ids, start_date, end_date)
